...
class XarmClient:
    ...
    async def get_status(self) -> Optional[dict]:
        return await self._get("/manipulator/status")
    
    async def get_current_position(self) -> XarmPositionResponse:
    ...
    # async with XarmClient("http://localhost:8000/api/v1/xarm") as client:
...
class IgusClient:
    ...
    async def move_to_position(
        ...
        """
        url = f"{self.base_url}/api/v1/igus/motor/move"
        payload = {
            "position": position,
            "velocity": velocity,
    ...
    async def get_task_status(self, task_id: str) -> Dict[str, Any]:
        ...
        """
        url = f"{self.base_url}/api/v1/igus/motor/task_status/{task_id}"
        async with self._session.get(url) as resp:
            data = await resp.json()
            if resp.status != 200:
    ...
    async def healthcheck(self) -> bool:
        ...
        Simple health check.
        """
        url = f"{self.base_url}/api/v1/igus/health"
        async with self._session.get(url) as resp:
            data = await resp.json()
