...
async def script_operator(stop_event, data):
    ...
    try:
        if command == "box_1":
            ...
            pose = xarm_positions.poses[data['position_name']]
            return await changePosition(stop_event, pose, speed=int(data['speed']))
...
async def transport_position(stop_event, speed=default_speed):
    try:
        async with igus_client as _igus_client:
            igus_result = await _igus_client.move_to_position(20000,speed,speed,wait=True)
            if not igus_result.get("success", False):
            ...
            async with xarm_client as client:
                ...
                igus_result2 = await _igus_client.move_to_position(0,speed,speed,wait=True)
...
async def goToBox1(stop_event, speed = default_speed):
    ...
    try:
        async with igus_client as _igus_client:
            igus_result = await _igus_client.move_to_position(first_pos, speed, speed, True)
            if not igus_result.get("success", False):
                error_msg = igus_result.get("error", "Igus move failed")
                logger.error(f"Igus movement failed: {error_msg}")
                raise Exception(error_msg)
            ...
            if result < 250:
                async with xarm_client as _xarm_client:
                    ...
                    igus_result = await _igus_client.move_to_position(int(second_pos), int(speed/2), int(speed/2), False)
...
async def changePosition(stop_event, position, speed = default_speed):
    try:
        if "SECTION" in position['name']:
            async with xarm_client as client:
                robot_result = await client.go_to_position([xarm_positions.poses["READY_STEP_1"],xarm_positions.poses["READY_STEP_2"],position],angle_speed=speed)
                if not robot_result:
